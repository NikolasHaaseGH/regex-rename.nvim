...
function M.add_with_visual_area(lnum, col, curswant, visual_start_lnum, visual_start_col, add_seq, tokenLength)
    ...
    -- Create an extmark
    extmark.update_virtual_cursor_extmark(virtualCursors[#virtualCursors], tokenLength, true)
end

-- Add a new virtual cursor
-- add_seq indicates that a sequence number should be added to store the order that cursors have being added
function M.add(lnum, col, curswant, add_seq, tokenLength)
    M.add_with_visual_area(lnum, col, curswant, 0, 0, add_seq, tokenLength)
end
